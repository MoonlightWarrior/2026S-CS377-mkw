...
test results, and the planned experimental protocol for the two research
questions.
\end{abstract}
...
agents plus eight CPUs, ranks 8--12), whereas the proposal's ``rank~1 vs.\
rank~4'' analysis assumes a clean four-kart race; we therefore target a CPUs-off
save state. (4)~The memory reader is occasionally not ready immediately after a
...
track in a clean 2v2 (CPUs off) with items enabled.
...
Second, a clean ``rank~1 vs.\ rank~4'' analysis requires a four-kart (CPUs-off)
save state, which we are preparing. The hit/teamkill metric for RQ2 is the most
uncertain instrumentation and may rely on the speed-drop proxy. None of these
